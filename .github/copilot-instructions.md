...
4. Read `docs/ARCHITECTURE_FOUNDATION.md`. A foundation freeze is active: do not suggest product features, schema additions, public integrations, or new critical cross-module flows until #17, #19, and #21 are closed.
...
- Treat application scoping as defense in depth only: PostgreSQL RLS is not currently proven. Follow the #21 transaction-scoped RLS design in `docs/ARCHITECTURE_FOUNDATION.md` before claiming database isolation.
...
- Run `pnpm architecture:check` before accepting API changes. The legacy in-process emitter is not approved for new critical business effects until transactional outbox #17 lands.
- Extension manifests must validate `apiVersion` through `@unerp/service-kit`'s published compatibility range; do not add a public contract or retire a supported version without updating `docs/EXTENSION_SERVICE_CONTRACT.md` and its focused tests.
