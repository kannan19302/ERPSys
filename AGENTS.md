...
10a. **Cross-platform parity is binding.** [.ai/MULTI_CLIENT_MASTER_PLAN.md](.ai/MULTI_CLIENT_MASTER_PLAN.md) was SEALED v1.0 on 2026-07-28, same permanent status as `ARCHITECTURE_FOUNDATION.md` — no agent may edit it. Every module now grows on Web + Mobile (iOS/Android) + Desktop (Windows/macOS/Linux) together, per its Tier 1-4 client-applicability classification (§ 5 there); this is the 6th binding module-completion criterion in `.ai/MODULE_REGISTRY.md` § 0, alongside the original 5. The 4 externalized vertical apps stay separate microservices behind the extension gateway (§ 2 there) — do not merge them into `apps/api`.
