...
- **UI from the UniERP Design System only** (`@unerp/ui-*` packages via the `@unerp/ui` facade) + `.ui-*` utility classes. **No** inline styles for forms/layout, **no** hardcoded pixels/hex — tokens from `@unerp/ui-tokens`. If a primitive is missing, coordinate with uiux-designer to add it to the right `packages/ui-*` package.
