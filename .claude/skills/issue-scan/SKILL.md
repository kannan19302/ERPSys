...
3. **Multi-collaborator gate** (AUTOPILOT § Multi-Collaborator Focus Selection): at
   bootstrap run `node scripts/claim.mjs list`. If other sessions hold active claims
   and a user is present, ask which module to scan deeply; if they pick a module an
   active collaborator holds, say who holds it and ask them to choose another (their
   in-flight, uncommitted work would produce false findings). Never file issues
   against another session's in-progress scope. Unattended: scan only unclaimed
   modules and say so in the report.
4. Dedupe every candidate against open issues in `kannan19302/ERPSys` before filing.
5. File severity-labeled issues, one problem (or one workflow/surface checklist) per
   issue, using the agent's template (What / Evidence / Impact / Suggested fix / Source).
6. Report a compact table: filed, skipped duplicates, overflow remaining.
