...
`apiVersion` is normalized and validated against the explicit compatibility range exported by `@unerp/service-kit`: `MIN_SUPPORTED_EXT_API_VERSION` through `EXT_API_VERSION`. Core rejects malformed, retired, and newer versions. When a future contract version ships, core retains the prior supported version for one release before raising the minimum; extensions must publish a compatible bundle before that retirement.
...
- Breaking changes to the token or envelope bump `EXT_API_VERSION`. Core supports the range from `MIN_SUPPORTED_EXT_API_VERSION` through `EXT_API_VERSION`; only after a documented release window may the minimum move forward.
