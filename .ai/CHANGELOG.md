...
- **Analytics (1,505→1,520+ features)**: KPI Definition Library (CRUD with formulas, categories, visualization config, targets), Trend Analysis (period-over-period KPI trends with MONTHLY/QUARTERLY/YEARLY aggregation, change percent computation, trend direction), Scheduled Exports (configurable export scheduling with format/source/recipients/preview), Cross-Filter Dashboards (interactive dashboards with layout/widgets/global filters), BI Metric Catalog (enterprise metric definitions with source/expression/dimensions). `AnalyticsDeepService` (384 LOC, 22 methods) + `AnalyticsDeepController` (22 endpoints). 5 new Prisma models (`AnalyticsKpiDefinition`, `AnalyticsTrendResult`, `AnalyticsScheduledExport`, `AnalyticsCrossFilterDashboard`, `AnalyticsBiMetricDefinition`). 11 new permissions. 5 UI pages (`/analytics/kpis`, `/analytics/trends`, `/analytics/exports`, `/analytics/catalog`). 19 Vitest tests. Additionally fixed 12 pre-existing schema validation errors by adding missing Prisma back-references (`Document.approvals`, `DriveFolder.shares`, `DriveFile.tagMappings+trashItems`, `MarketplaceApp.versions+analyticsRecords`, `User.apiTokens`, `Tenant.aiModelAccuracyMetrics`, `Tenant.aiIntentTrainingExamples`, `Tenant.analytics*`). Schema applied via `prisma db push`.

## [2026-07-27] Cycle 62 — Auth, Marketplace & Blockchain Module Deepening

- **Auth (40→60+ features)**: API Token management (create/revoke with 64-char tokens), Login History viewer (paginated/filtered), Session management (list/force-revoke), Password Policy settings (min length/expiry/complexity via tenant settings), IP Allowlist/Restriction CRUD (CIDR whitelist/blacklist). `AuthDeepService` (~400 LOC) + `AuthDeepController` (15 endpoints) at `apps/api/src/modules/auth/`. `AuthApiToken` Prisma model. 12 new permissions. 4 UI pages (API Tokens, Login History, Sessions, Security). 1 test file (`auth-deep.service.spec.ts`).
- **Marketplace (51→70+ features)**: App Reviews (CRUD + aggregate rating stats + duplicate-review guard), App Version History (list/create releases), Developer Submission workflow (submit→approve/reject with status transitions), Analytics dashboard (top apps by installs/revenue, aggregate totals). `MarketplaceDeepService` (~300 LOC) + `MarketplaceDeepController` (12 endpoints) at `apps/api/src/modules/marketplace/`. 3 new Prisma models (`MarketplaceAppVersion`, `MarketplaceDeveloperSubmission`, `MarketplaceAnalytics`). 9 new permissions. 3 UI pages (Reviews, Submissions, Analytics). 1 test file.
- **Blockchain (11→30+ features)**: Transaction Explorer (search by hash/address/block, paginated), Smart Contract registry (CRUD with network/ABI metadata), Append-only Audit Trail (paginated, filter by entity type, tx hash proof), Network Health dashboard (multi-network monitoring). `BlockchainDeepService` (~300 LOC) + `BlockchainDeepController` (12 endpoints) at `apps/api/src/modules/blockchain/`. 4 new Prisma models (`BlockchainSmartContract`, `BlockchainAuditTrail`, `BlockchainNetworkHealth`, `BlockchainTransactionExplorer`). 10 new permissions. 4 UI pages (Transactions, Contracts, Audit, Network). 1 test file.

## [2026-07-27] Cycle 61 — Documents & Drive Module Deepening

- **Documents (21→50+ features)**: Document Template Library with variable substitution + render endpoint, Document Categories with tree structure (parentId self-ref), Bulk Upload endpoint, Approval Routing workflow (submit/approve/reject), OCR placeholder endpoint, Version Diff Viewer. 6 new Prisma models/fields (DocumentCategory, DocumentApproval, DocumentTemplate.category+variables, DocumentVersion.changes). 1 new service (DocumentsDeepService, ~350 LOC), 1 new controller with 16+ endpoints, DTOs file, 3 UI pages (templates, categories, approvals), unit tests. 20+ new permission entries.
- **Drive (23→50+ features)**: Folder Sharing (share folder with specific users), File Preview metadata endpoint, Trash/Restore with DriveTrashItem tracking + auto-purge after 30 days, Storage Usage Reports (per-user/per-folder breakdown + quota + type breakdown), File Tagging (CRUD tags + assign/unassign to files), Direct Download Links (expiring tokens). 4 new Prisma models (DriveFolderShare, DriveFileTag, DriveFileTagMapping, DriveTrashItem). 1 new service (DriveDeepService, ~580 LOC), 1 new controller with 16+ endpoints, DTOs file, 2 UI pages (usage, tags), unit tests. 10+ new permission entries.
...
## [2026-07-27] Cycle 61 — Healthcare & Education Module Deepening (MVM→COMPLETE) [SUPERSEDED BY CYCLE 60 ENTRY ABOVE]
...
## [2026-07-27] Cycle 60 — Four Modules Skeleton→Functional (DevOps Pipeline, PWA, Outbox Deepening, Ext Gateway) [SUPERSEDED BY CYCLE 60 ENTRY ABOVE]
...
- Rewrote both `nav` arrays to flat 12-item top-level lists (0 `isHeader`), each mirroring the module's already-built `*_TABS` primary tab bar (`CrmTabLayout`/`InventoryTabLayout` in `apps/web/src/components/{crm,inventory}/`).
- **CRM** (12 items: Dashboard, Leads, Opportunities, Customers, Contacts, Sales Pipeline, Marketing, Automation, Customer Success, Intelligence & AI, Reports & Analytics, Settings): `CRM_TABS` only had 9 entries (5 primary + 4 advanced) before this change — far short of covering the old sidebar's 62 routes. Extended `CRM_TABS` (`apps/web/src/components/crm/CrmTabLayout.tsx`) with 53 new `advanced: true` tabs across 9 new groups (Account Management, Sales Pipeline, Automation & Workflows, Customer Service, Sales Enablement, Teams & Territories, Analytics & Reports, CRM Intelligence & AI, plus Settings detail pages) so every one of the original 62 sidebar routes is now reachable via the tab bar's "Advanced" dropdown. The flat sidebar itself carries the 5 pre-existing primary tabs, the 4 pre-existing advanced tabs (Marketing/Automation/Customer Success/Settings), and 3 new group-representative shortcuts (Sales Pipeline → `/crm/quotations`, Intelligence & AI → `/crm/intelligence`, Reports & Analytics → `/crm/reports`) for the newly-added groups. Settings sidebar entry deliberately points at `/crm/settings/custom-fields` rather than `/crm/settings` — the latter has no `page.tsx` (subpages only, pre-existing gap, not introduced here).
- **Inventory** (12 items: Dashboard, Products Catalog, Stock Levels, Warehouses, Material Transactions, Quality & Traceability, Counting & Storage, Warehouse Operations, Transfers & Logistics, Analytics & Planning, Returns & Movement, Settings): `INVENTORY_TABS` already had full 58-route coverage via 7 pre-existing groups (Quality & Traceability, Counting & Storage, Warehouse Operations, Transfers & Logistics, Analytics & Planning, Returns & Movement, Settings) — untouched. Sidebar mirrors the 5 primary tabs plus one representative shortcut per advanced group.
- No routes deleted; no page files removed. Every original sidebar destination remains reachable as a flat sidebar item or via the tab bar's Advanced dropdown (verified present on every page in the module via `apps/web/app/(dashboard)/{crm,inventory}/layout.tsx`).
- **Verified** (literal command output): `grep -c "href:" apps/web/src/navigation/descriptors/crm.ts` → 12, `grep -c "isHeader"` → 0. Same for `inventory.ts` → 12 / 0. `pnpm --filter @unerp/web typecheck` → clean, 0 errors. `pnpm architecture:check` → module boundary check passed, depcruise "no dependency violations found (1194 modules, 5465 dependencies cruised)". Live browser verification on `web-alt` (port 3010, demo Super Admin login): `/crm` renders the 12-item sidebar and 5-tab bar + Advanced dropdown; `/crm/settings/custom-fields` resolves with breadcrumb `Apps / CRM / Settings / Custom Fields`; `/inventory` renders the 12-item sidebar and 5-tab bar + Advanced dropdown; `/inventory/warehouse-ops` resolves with breadcrumb `Apps / Inventory / Warehouse Operations`.

## [2026-07-24] Real sidebar flattening: Real Estate, AI, Analytics, Builder

**Follows up the GOVERNANCE correction and the HR/Drive/Manufacturing/Projects remediation above** — same failure mode confirmed a third time: `apps/web/src/navigation/descriptors/{realestate,ai,analytics,builder}.ts` were byte-identical to their original deep/grouped (`isHeader: true`) state despite the 2026-07-21 changelog entry claiming they were done.

- Rewrote all 4 `nav` arrays to flat top-level lists (0 `isHeader`), each mirroring its module's existing `*_TABS` primary tab bar (`RealEstateTabLayout`/`AiTabLayout`/`AnalyticsTabLayout`/`BuilderTabLayout` in `apps/web/src/components/<module>/`).
- **Real Estate** (8 items): added `Agent Commissions` and `Settings` to `REAL_ESTATE_TABS` (`apps/web/src/components/real-estate/RealEstateTabLayout.tsx`) — both had real pages (`/real-estate/commissions`, `/real-estate/settings`) but no tab.
- **AI** (7 items): added `Settings` tab (`/ai/settings`) to `AI_TABS`. Dropped the old descriptor's stray `Visual Query Builder → /analytics/query` entry — that route belongs to, and is already covered by, the Analytics module's own descriptor. **Also fixed a real functional bug found in the process**: `apps/web/app/(dashboard)/ai/page.tsx` never read the `?tab=` query param that `AI_TABS`' deep-links (`/ai?tab=invoice-scanner` etc.) already relied on, so every tab link silently opened the same default "Ask Data" mode. Added a `TAB_PARAM_TO_MODE` map and `useSearchParams()`-derived initial state so those links now actually work.
- **Analytics** (12 items): 6 of the module's 12 page routes (`dashboards`, `advanced`, `reports`, `kpis`, `exports`, `settings`) existed as real pages but had neither a sidebar entry nor a tab. Added them all to `ANALYTICS_TABS` (`apps/web/src/components/analytics/AnalyticsTabLayout.tsx`) as `advanced: true` / `group: "More Analytics"` overflow tabs, and to the flat sidebar.
- **Builder** (4 items): mirrors `BUILDER_TABS`' 4 Studio-pillar tabs exactly (Studio Home / App Studio / Web Studio / Manage) — Builder's ~30 sub-routes stay reachable via each pillar's own pre-existing Level-2 `SubTabBar` (`erp-sub-tabs.ts`/`web-sub-tabs.ts`/`manage-sub-tabs.ts`), which is the module's real "already-built tab bar" per `BuilderTabLayout.tsx`'s own header comment. Added the one genuinely missing route, `erp/modules` ("Custom Apps"), to `ERP_SUB_TABS`. Dropped the old descriptor's "Marketplace" group (`/apps/store`, `/apps`, `/apps/developer`) entirely — those are already a separate, already-flat top-level nav module (`apps/web/src/navigation/descriptors/appStore.ts`), not part of Builder.
- No routes deleted; no page files removed. Every original sidebar destination remains reachable as a flat sidebar item, via a tab bar's Advanced dropdown, or (Builder only) via a pillar's SubTabBar.
- **Verified** (literal command output): `grep -c "href:" apps/web/src/navigation/descriptors/<module>.ts` → real-estate 8, ai 7, analytics 12, builder 4 (all ≤15); `grep -c "isHeader"` → 0 for all 4. `pnpm --filter @unerp/web typecheck` → 0 errors. `pnpm architecture:check` → module boundary check passed, depcruise 0 violations (1194 modules, 5465 dependencies). Live browser verification was not possible this session — the shared preview instance (port 3010) was under active navigation by other concurrent agent sessions per the multi-collaborator protocol; relied on static verification (typecheck, architecture check, and direct code reading of the `?tab=`/mode-mapping logic) instead.
