...
| **Advanced**                    | 500–1000       | Exceeds most market leaders        | Normal priority                                               |
| **Complete**                    | 1000–1500      | Near completion bar                | Normal priority                                               |
| **Deep**                        | 1500+          | Completion bar met                 | Maintenance mode                                              |

**Current distribution**: see `.ai/MODULE_REGISTRY.md` § System Progress Dashboard (regenerated by `node scripts/module-health.mjs` — do not hand-maintain counts here). The priority ladder deprioritizes deepening modules above 200 until all modules reach MVM.
...
Module completion target: **1500 weighted feature points** (not raw endpoint count).

### 8.3 Module Completion Definition

A module is **COMPLETE** when it has:

- **1500+ weighted feature points** (verified by `node scripts/feature-ledger.mjs`)
...
**Binding throughput floor (Phase M/X):** every DEV cycle ships
**≥ 5,000 net LOC OR ≥ 40 distinct features** before Record + Ship. Net LOC =
adds + modifies from `git diff --numstat <cycle-start-sha>..HEAD` excluding
lockfiles/generated files (`node scripts/cycle-report.mjs` computes it). Under
both floors → extend the cycle with the next priority-ladder item; do not ship
short without a hard stop (budget exhaustion, red gate needing human input),
stated explicitly in the report. Phase F and harden cycles are exempt.

| Metric                       | Minimum Target                                    |
| :--------------------------- | :------------------------------------------------ |
| **Throughput floor**         | 5,000+ net LOC OR 40+ business features per cycle |
| **End-to-end slices**        | Every feature = DB schema + API + UI + tests      |
| **Test coverage**            | 80%+ on all new code                              |
| **Feature ledger delta**     | Matches the shipped feature count in the ledger   |
| **Micro-harden checkpoints** | At features 20, 40, 60: scoped typecheck + vitest |
...
- Every cycle report MUST include a feature count delta AND the measured net LOC
- The Cycle Ledger's `Net LOC` column is filled for every DEV cycle row
- If a DEV cycle ships under BOTH floors (5,000 net LOC / 40 features), the
  cycle report must name the hard stop that excused it
- The plan at cycle start must identify enough scope to clear the floor
...
1. **Regenerate ONCE per cycle, at Record** (`node scripts/feature-ledger.mjs`) —
   this captures the cycle's new work; do not also regenerate at cycle start
2. **At cycle start, grep the EXISTING ledger** for duplicate-checks —
   regenerate early only if the file is missing or clearly stale
...
- **Cache validity**: 30 days per module section
...
- Fresh research only required when the module section is > 30 days old
...
| **`.ai/ARCHITECTURE_FOUNDATION.md`** | Sealed architecture contracts (foundation SEALED 2026-07-18)  | Before architecture changes   |
...
║    4. Claim your scope                                           ║
║    5. Grep existing feature ledger for duplicates                ║
║                                                                  ║
║  WHILE WORKING:                                                  ║
...
║    • 5K+ net LOC OR 40+ features per DEV cycle; 10+ QA fixes     ║
║                                                                  ║
║  AFTER WORK:                                                     ║
