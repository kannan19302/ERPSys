...
39. **[benchmark] CRM: notification consumer for `pipeline.deal.at_risk`** — this cycle's `CrmPipelineRiskService` emits the event on every new alert but nothing subscribes yet (mirrors the `finance.invoice.overdue` gap that was closed in item 25d — same fix pattern: an `@OnEvent` listener resolving tenant users with a CRM permission via `Role.permissions`). Leaders: Salesforce (Chatter alerts), HubSpot (Slack/email). RICE: Reach 45 · Impact 6 · Confidence 65% · Effort 2 = **135**.
...
41. **[benchmark] CRM: AI-assisted email/quote drafting (generative sales content)** — Salesforce Einstein GPT / HubSpot Breeze Copilot draft follow-up emails and quote cover notes from deal context. Leaders: Salesforce Einstein GPT, HubSpot Breeze, Zoho Zia. RICE: Reach 45 · Impact 6 · Confidence 40% · Effort 6 = **18**.
42. **[benchmark] CRM: revenue intelligence / deal-risk email digest** — daily/weekly digest email to reps/managers summarizing new pipeline risk alerts (item 38) and at-risk deal counts, reusing the existing `notification.send` + dunning-digest pattern. Leaders: Gong, Clari. RICE: Reach 40 · Impact 5 · Confidence 55% · Effort 3 = **37**.
43. **[benchmark] CRM: lead-to-opportunity conversion analytics dashboard** — funnel conversion-rate reporting by lead source/campaign, deepening `crm-dashboards.service.ts`. Leaders: Salesforce, HubSpot. RICE: Reach 40 · Impact 5 · Confidence 60% · Effort 3 = **40**.
