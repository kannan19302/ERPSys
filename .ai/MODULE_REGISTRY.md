...
6. **[benchmark] Finance: lease accounting (ASC 842/IFRS 16)** — right-of-use asset + lease liability schedule, straight-line/effective-interest amortization, auto-generated monthly journal entries; leaders: NetSuite Fixed Assets, Sage Intacct Lease Accounting. Value H, Size M. Sub-tasks: `Lease`/`LeaseSchedule` Prisma models, amortization engine, `/leases` CRUD + `/leases/:id/schedule`, monthly posting job, UI wizard. RICE: Reach 50 · Impact 9 · Confidence 75% · Effort 3 = **112.5**.
7. **[benchmark] Finance: project-based accounting (WIP, job costing, percentage-of-completion)** — tie `RevenueSchedule` to Projects for job-costing and POC revenue recognition; leaders: Sage Intacct, Deltek, NetSuite SRP. Value H, Size M. Sub-tasks: `ProjectCostEntry` model, WIP report, POC recognition formula (costs-incurred / total-estimated-cost × contract value), `/projects/:id/wip` endpoint, UI report. RICE: Reach 40 · Impact 9 · Confidence 70% · Effort 3 = **84**.
8. **[benchmark] Finance: subscription billing & recurring-revenue metrics (MRR/ARR)** — usage-based billing lines, MRR/ARR/churn dashboard tied to `RevenueSchedule`; leaders: Sage Intacct, Zuora, NetSuite SuiteBilling. Value H, Size M. Sub-tasks: `Subscription`/`SubscriptionLine` models, billing-run job, MRR/ARR/churn calc service, `/subscriptions` CRUD + `/subscriptions/metrics`, dashboard UI. RICE: Reach 45 · Impact 9 · Confidence 70% · Effort 3 = **94.5**.
9. **[benchmark] Finance: unified spend management (real-time card controls)** — extend corporate cards (shipped 2026-07-08) with budget-linked spend limits and live transaction approval/decline rules; leaders: Ramp, Brex, Emburse, BILL Spend & Expense. Value M, Size M. Sub-tasks: `CardSpendLimit` model (per-card/per-category monthly cap), pre-authorization check service, `/corporate-cards/:id/limits` CRUD, limit-utilization widget on expense-policies page. RICE: Reach 35 · Impact 7 · Confidence 70% · Effort 2 = **85.75**.
10. **[benchmark] Finance: continuous close automation (variance flagging, close-task assignment)** — extend the existing financial-periods close checklist with automated balance-variance flags and assignable close tasks; leaders: Sage Intacct, BlackLine, FloQast. Value M, Size M. Sub-tasks: `CloseTask` model (assignee, due date, status), variance-flag rule (period-over-period % threshold), `/financial-periods/:id/close-tasks` CRUD, close dashboard widget. RICE: Reach 30 · Impact 7 · Confidence 70% · Effort 2 = **73.5**.
11. **[benchmark] CRM: customer self-service portal** — customer login to view quotes/orders/invoices/tickets; leaders: Odoo, Zoho, Dynamics (see `.ai/MARKET_BENCHMARK.md` § CRM). Value H, Size L — defer to CRM focus turn.
