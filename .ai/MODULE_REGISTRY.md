...
6. **[benchmark] Finance: AP three-way matching** — auto-match PO → goods receipt → vendor invoice with configurable tolerance rules (quantity ±%, price ±%), exception queue for out-of-tolerance invoices, auto-post GL when matched; leaders: NetSuite (per-vendor tolerance, auto-post), SAP S/4HANA (LIV posting, price variance GL), Dynamics 365, Odoo. Sub-tasks: `APMatchRule` model (tolerances), matching engine service, `/payables/invoices/:id/match` endpoint, exception-queue page. RICE: Reach 55 · Impact 8 · Confidence 75% · Effort 3 = **110**.
7. **[benchmark] Finance: financial statement drill-through** — click any P&L / Balance Sheet line total → see underlying journal entries → click to source document (invoice, payment, GR); leaders: NetSuite SuiteAnalytics, Sage Intacct, Dynamics 365. Sub-tasks: drill-through query service (`/advanced-finance/reports/:reportType/drilldown?account=&period=`), click handler + slide-over panel in P&L and BS report pages. RICE: Reach 70 · Impact 7 · Confidence 90% · Effort 1 = **441** (small effort, high reach — a UX bridge over existing endpoints).
8. **[benchmark] Finance: batch vendor payment run** — select open AP invoices → generate payment batch → output bank file (SEPA XML / NACHA ACH) + remittance advice PDFs → post payment GL entries in bulk; leaders: SAP, NetSuite, Dynamics, Odoo. Sub-tasks: `PaymentBatch`/`PaymentBatchLine` models, batch-assembly service, `/payables/payment-batches` CRUD + export, run-summary UI page. RICE: Reach 50 · Impact 8 · Confidence 75% · Effort 3 = **100**.
9. **[benchmark] Finance: subscription billing & recurring-revenue metrics (MRR/ARR)** — usage-based billing lines, MRR/ARR/churn dashboard tied to `RevenueSchedule`; leaders: Sage Intacct, Zuora, NetSuite SuiteBilling. Value H, Size M. Sub-tasks: `Subscription`/`SubscriptionLine` models, billing-run job, MRR/ARR/churn calc service, `/subscriptions` CRUD + `/subscriptions/metrics`, dashboard UI. RICE: Reach 45 · Impact 9 · Confidence 70% · Effort 3 = **94.5**. *[claimed by antigravity-ide 2026-07-08]*
10. **[benchmark] Finance: unified spend management (real-time card controls)** — extend corporate cards (shipped 2026-07-08) with budget-linked spend limits and live transaction approval/decline rules; leaders: Ramp, Brex, Emburse, BILL Spend & Expense. Value M, Size M. Sub-tasks: `CardSpendLimit` model (per-card/per-category monthly cap), pre-authorization check service, `/corporate-cards/:id/limits` CRUD, limit-utilization widget on expense-policies page. RICE: Reach 35 · Impact 7 · Confidence 70% · Effort 2 = **85.75**.
11. **[benchmark] Finance: project-based accounting (WIP, job costing, percentage-of-completion)** — tie `RevenueSchedule` to Projects for job-costing and POC revenue recognition; leaders: Sage Intacct, Deltek, NetSuite SRP. Value H, Size M. Sub-tasks: `ProjectCostEntry` model, WIP report, POC recognition formula (costs-incurred / total-estimated-cost × contract value), `/projects/:id/wip` endpoint, UI report. RICE: Reach 40 · Impact 9 · Confidence 70% · Effort 3 = **84**.
12. **[benchmark] Finance: continuous close automation (variance flagging, close-task assignment)** — extend the existing financial-periods close checklist with automated balance-variance flags and assignable close tasks; leaders: Sage Intacct, BlackLine, FloQast. Value M, Size M. Sub-tasks: `CloseTask` model (assignee, due date, status), variance-flag rule (period-over-period % threshold), `/financial-periods/:id/close-tasks` CRUD, close dashboard widget. RICE: Reach 30 · Impact 7 · Confidence 70% · Effort 2 = **73.5**.
13. **[benchmark] CRM: customer self-service portal** — customer login to view quotes/orders/invoices/tickets; leaders: Odoo, Zoho, Dynamics (see `.ai/MARKET_BENCHMARK.md` § CRM). Value H, Size L — defer to CRM focus turn.
