...
5w. Otherwise-exhausted named gaps: ZPL/PDF barcode label rendering (needs a barcode-symbology dependency decision), mobile-optimized scan-first UI (touch/camera-scan layout). Next cycle: run another discovery pass on a different Inventory sub-domain (e.g. labor management, demand forecasting, returns-to-vendor workflow).
