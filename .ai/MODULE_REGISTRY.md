...
5e. **Inventory: kitting/BOM assembly-disassembly deepening** — `ProductKit`/`ProductKitItem` exist as basic CRUD; add assemble/disassemble stock-entry generation, component availability check, kit costing rollup. Part of the 90→200 feature-count plan (see `MODULE_FOCUS.md` §6, 2026-07-12).
...
5g. **Inventory: multi-warehouse transfer approval workflow** — approval step on `StockEntry` MATERIAL_TRANSFER type above a configurable value/quantity threshold. Part of the 90→200 plan.
5h. **Inventory: inventory audit-trail / movement-history report** — consolidated per-product/per-warehouse movement timeline across stock entries, cycle counts, put-away, and reservations. Part of the 90→200 plan.
5i. **Inventory: vendor-managed / consignment inventory** — supplier-owned stock held at tenant warehouses, consumption-triggered billing. Part of the 90→200 plan.
5j. **Inventory: barcode label printing** — generate/print SKU, batch, license-plate, and bin barcode labels (ZPL/PDF). Part of the 90→200 plan.
