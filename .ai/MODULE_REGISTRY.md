...
5u. **[benchmark] Inventory: dynamic slotting optimization** — confirmed gap via the same 2026-07-12 discovery pass: Manhattan Active WMS analyzes real-time order data to suggest optimal bin reassignment (fast-movers near packing, slow movers to reserve zones) on a recurring cadence; UniERP has bin-suggestion for put-away (cycle 1) but no periodic re-slotting recommendation engine. RICE: Reach 40 · Impact 6 · Confidence 50% · Effort 5 = **24**.
5v. Otherwise-exhausted named gaps: ZPL/PDF barcode label rendering (needs a barcode-symbology dependency decision), mobile-optimized scan-first UI (touch/camera-scan layout). Next cycle: build 5u, or run another discovery pass on a different Inventory sub-domain (e.g. yard/dock scheduling, labor management).
