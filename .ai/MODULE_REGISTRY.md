...
5q. **Inventory: mobile-optimized scan-first pick/pack UI** — remainder of item 5d; the desktop-oriented `/inventory/pick-waves` and `/inventory/license-plates` pages exist, a dedicated scan-first mobile flow does not. Part of the 90→200 plan.
...
5s. Backlog is otherwise exhausted — next cycle should run a fresh `MARKET_BENCHMARK.md` discovery pass on Inventory & Supply Chain (P5) if 5p/5q (barcode rendering, mobile scan UI) are deferred, or module-deepen (P6) an existing area.
