...
24. **[e2e-unverified] Finance: 1099 / vendor tax reporting** — Docker/Postgres were started in the 2026-07-11 session only for the Prisma migration, not the full stack; the Playwright smoke gate has not been run against `/finance/advanced/1099-reporting` yet. Next session with the full stack up: add the route to `SMOKE_ROUTES` and verify.
...
25. **[benchmark] Finance: quality-gap closeout for 500-feature exit criteria** — `FEATURE_LEDGER.md` now counts 515 Finance+Advanced-Finance features (crossed the 500 target 2026-07-11), but `MODULE_FOCUS.md` § 5 exit criteria also require scorecard 10/10 on all 7 dimensions, the module's pages green in the E2E smoke suite, zero unresolved `FEEDBACK.md` errors, published integration contracts, and a UAT pass — confirm each before advancing focus to CRM & Sales.
26. **[benchmark] Finance: automated sales/use-tax nexus & economic-nexus monitoring** — per-state registration-threshold tracking + real-time multi-jurisdiction rate lookup, extending the existing `TaxEngineService`/`TaxEngineDeepService` (which only compute flat/withholding tax today). Leaders: Avalara AvaTax, Vertex, NetSuite SuiteTax. RICE: Reach 60 · Impact 6 · Confidence 60% · Effort 3 = **72**.
