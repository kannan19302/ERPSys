...
25c. **[exit-criteria] Finance: UAT pass** — run a `business-analyst-uat`-style walkthrough of the module's top 10 business workflows (invoice-to-cash, procure-to-pay, month-end close, budget-to-actual, 1099 filing, etc.) against the running app and record the sign-off in CHANGELOG. Directly blocks §5 criterion 6. **Still open — next cycle**, needs live dev stack.
25d. **[exit-criteria] Finance: wire a real consumer for `finance.invoice.overdue`** — event is emitted (`tax-engine.service.ts:866`) but has zero listeners; Notifications module (or CRM dunning) should subscribe via `@OnEvent('finance.invoice.overdue')`. Small, well-scoped follow-up identified by the 2026-07-11 contract audit.
