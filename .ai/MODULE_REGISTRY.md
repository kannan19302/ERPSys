...
25a. **[exit-criteria] Finance: full E2E smoke sweep of all Finance/advanced-finance pages** — only 1099-reporting and tax-nexus pages are currently in `SMOKE_ROUTES`; every other Finance/advanced-finance page (GL, AR/AP, budgets, leases, subscriptions, treasury, fixed assets, consolidation, etc.) needs a smoke route + a live-stack Playwright pass. Directly blocks MODULE_FOCUS.md §5 criterion 3.
25b. **[exit-criteria] Finance: publish § 7 integration contracts** — `MODULE_FOCUS.md` §7 lists `finance.invoice.posted`, `finance.payment.received`, `finance.invoice.overdue`, `sales.order.confirmed` (consumes), `purchase.received` (consumes), `hr.payroll.run.completed` (consumes), `inventory.valuation.changed` (consumes), `pos.session.closed` (consumes) all as "planned"/"partially wired" — verify/implement each emitter+consumer pair and mark as published. Directly blocks §5 criterion 5.
25c. **[exit-criteria] Finance: UAT pass** — run a `business-analyst-uat`-style walkthrough of the module's top 10 business workflows (invoice-to-cash, procure-to-pay, month-end close, budget-to-actual, 1099 filing, etc.) against the running app and record the sign-off in CHANGELOG. Directly blocks §5 criterion 6.
