...
5v. Otherwise-exhausted named gaps: ZPL/PDF barcode label rendering (needs a barcode-symbology dependency decision), mobile-optimized scan-first UI (touch/camera-scan layout). Next cycle: run another discovery pass on a different Inventory sub-domain (e.g. yard/dock scheduling, labor management, demand forecasting).
