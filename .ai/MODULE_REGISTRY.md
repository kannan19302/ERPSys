...
10. **[benchmark] Finance: continuous close automation (variance flagging + close-task assignment)** — extend the existing financial-periods close checklist with automated period-over-period variance flags and assignable close tasks (assignee, due date, checklist status); leaders: Sage Intacct (Close Automation + Subledger Reconciliation Assistant), NetSuite (Intelligent Close Manager), BlackLine, FloQast. Sub-tasks: `CloseTask` model (assignee, due date, status), variance-flag rule (period % threshold), `/financial-periods/:id/close-tasks` CRUD, close dashboard widget. RICE: Reach 50 · Impact 8 · Confidence 80% · Effort 2 = **160**.
...
12. **[benchmark] Finance: driver-based budgeting & scenario planning (FP&A)** — budget templates by driver (headcount × salary, units × price), multi-period spread, "what-if" scenario builder to test revenue/cost assumptions before committing; leaders: NetSuite Planning & Budgeting (NSPB), Dynamics 365 Business Performance, Workday Adaptive Planning. Sub-tasks: `BudgetScenario` model, scenario-clone + driver-compute service, scenario-comparison UI. RICE: Reach 55 · Impact 9 · Confidence 75% · Effort 3 = **123.75**.
13. **[benchmark] Finance: AI-powered invoice capture (OCR + auto-coding)** — upload vendor PDF invoice → OCR extracts vendor, amount, line items → auto-suggests GL account + cost centre + PO match → route for approval; leaders: NetSuite Bill Capture, Sage Intacct Intelligent AP, SAP Document Management. Sub-tasks: `/payables/invoices/capture` endpoint (simulated OCR extraction regex + ML classification stub), auto-GL-coding logic from vendor history, capture-queue UI page. RICE: Reach 65 · Impact 8 · Confidence 65% · Effort 2 = **169**.
14. **[benchmark] Finance: unified spend management (real-time card spend limits)** — extend corporate cards with budget-linked spend limits and per-transaction category controls; leaders: Ramp, Brex, Emburse, BILL Spend & Expense. Sub-tasks: `CardSpendLimit` model (per-card/per-category monthly cap), pre-authorization check service, `/corporate-cards/:id/limits` CRUD, limit-utilization widget. RICE: Reach 35 · Impact 7 · Confidence 70% · Effort 2 = **85.75**.
