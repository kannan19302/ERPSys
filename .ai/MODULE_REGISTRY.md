...
5a. **CRM & Sales exit-criterion 6 — finish the full Playwright smoke sweep of the 54 newly-added `SMOKE_ROUTES`** — the 2026-07-11 closeout cycle's bounded `timeout 280s` run only reached 39/138 total routes (dev-server per-route first-compile overhead, all within pre-existing Finance routes) before being cut off; it never reached the CRM/Sales routes. Manual curl-based UAT of the 5 core CRM/Sales workflows already PASSED live (see `.ai/UAT_CRM_2026-07-11.md`), but the automated regression sweep is `[e2e-partial]`. Next cycle: run against a production build (`next build && next start`, no per-route dev-compile tax) or split the sweep into smaller bounded chunks per route group, then update `MODULE_FOCUS.md` §5 criterion 6 and re-evaluate declaring CRM & Sales COMPLETE.
