...
| Gap | Who has it | UniERP | Value | Size |
|:--|:--|:--|:--|:--|
| Sales cadences / sequences with multi-channel touchpoints (call, email, task) auto-enrolled | Salesforce Sales Engagement, HubSpot | ✅ SHIPPED 2026-07-11 (`EmailSequenceStep.channel`, `CadenceAutoEnrollRule`, `CadenceStepTask`, due-step processor, `/crm/sequences/cadences` UI — see CHANGELOG) | H | M |
| Conversation intelligence (call logging + AI summary into Activity) | Salesforce Einstein, HubSpot | MISSING | M | M |
| Quote e-signature with legally-binding audit certificate | HubSpot, Zoho Sign, Odoo Sign | ✅ SHIPPED 2026-07-11 (`QuotationSignatureCertificate` — SHA-256 hash + structured audit trail + certificate number, `/crm/quotations/signatures` UI — see CHANGELOG) | M | S |
| Territory assignment rules engine (auto-routing by geo/size/round-robin) | Salesforce, Dynamics | ✅ SHIPPED 2026-07-11 (`TerritoryAssignmentRule`/`Log`/`RoundRobinState` — prioritized GEOGRAPHY/INDUSTRY/COMPANY_SIZE/ROUND_ROBIN, persisted round-robin cursor, `/crm/territories/assignment-rules` UI — see CHANGELOG) | H | M |
...
| CRM & Sales | 2026-07-11 (pass 2) | WebSearch (Salesforce/HubSpot/Dynamics 2026 CRM comparisons) | Pass 2: territory assignment rules engine, multi-channel sales cadences, quote e-signature audit certificate all SHIPPED (see CHANGELOG). WebSearch confirmed Salesforce Einstein Pipeline Inspection + HubSpot Breeze forecast-confidence scoring are now baseline enterprise expectations — logged as new gap (Up Next item 38, RICE 32). Remaining open: conversation intelligence, portal PDF download, portal online payment collection, pipeline inspection/stage-risk alerts (items 35-38). |
| CRM & Sales | 2026-07-11 (pass 3) | WebSearch ("2026 CRM conversation intelligence... Salesforce HubSpot Zoho comparison") | Pass 3: portal PDF download, portal online payment collection, pipeline inspection/stage-risk alerts (items 36-38) all SHIPPED (see CHANGELOG). WebSearch reconfirmed conversation intelligence (item 35) as still-live: 2026 market shifted from "transcribe calls" to "write structured deal data straight into the CRM" (Salesforce Einstein CI field-level write-back, HubSpot 2026 "Audit Cards" showing which CRM properties an AI agent changed, Zoho Zia sentiment analysis) — item 35 kept in Up Next at its existing RICE 14, deepened with 2 new sub-gap ideas (AI-assisted drafting item 41, revenue-intelligence digest item 42). Also logged: notification consumer for this cycle's own `pipeline.deal.at_risk` event (item 39, RICE 135 — cheapest item in the queue, should be picked next), real gateway wiring for portal payments (item 40, infra-blocked), lead-to-opportunity funnel analytics (item 43). |
| CRM & Sales | 2026-07-11 (pass 4) | WebSearch ("2026 CRM lead-to-opportunity conversion analytics funnel Salesforce HubSpot Zoho comparison") | Pass 4: notification consumer for `pipeline.deal.at_risk` (item 39), revenue-intelligence deal-risk digest (item 42), and conversation intelligence (item 35) all SHIPPED (see CHANGELOG cycle 4). WebSearch confirmed lead/opportunity funnel analytics (item 43) remains a real differentiator — Salesforce CRM Analytics (Einstein-scored likelihood-to-close + at-risk pipeline flagging) leads, HubSpot is strong on ease-of-use forecasting, Zoho's own reviews flag weak ROI/funnel attribution as a gap even in their own tier — confirming item 43 as a genuine, still-open competitive gap worth keeping at RICE 40. Remaining open in Up Next: real payment gateway wiring (item 40, infra-blocked), AI-assisted email/quote drafting (item 41), lead-to-opportunity funnel analytics (item 43, promoted to top of queue for next cycle). |
| CRM & Sales | 2026-07-11 (pass 5) | WebSearch ("2026 CRM sales forecasting AI features gap Salesforce HubSpot Zoho"; "CRM sales gamification leaderboards commission automation lead enrichment third-party data 2026") | Pass 5: lead-to-opportunity conversion analytics dashboard (item 43) and AI-assisted email/quote drafting (item 41) both SHIPPED (see CHANGELOG cycle 5). WebSearch found the 2026 CRM-AI landscape has converged — lead scoring/email generation/conversation intelligence/forecasting/enrichment/chatbots are now table-stakes across Salesforce/HubSpot/Zoho, so real differentiation is shifting to (a) sales performance management layered on top of the CRM (gamification/leaderboards — SalesScreen, Ambition, Spinify), (b) real-time waterfall data enrichment (Clay's 150+ provider waterfall, ZoomInfo, Clearbit), and (c) tiered/accelerator commission automation (Xactly, CaptivateIQ, Spiff) beyond flat-rate calc. Logged 3 new gaps: item 44 (gamification/leaderboards, RICE 58 — highest of the three, no infra dependency), item 45 (lead/contact enrichment, RICE 24 — partially infra-blocked on a real provider key but the queueing/caching/field-mapping layer is a pure code gap buildable against a mock provider), item 46 (commission plan automation deepening, RICE 18). Remaining open in Up Next: real payment gateway wiring (item 40, infra-blocked), items 44-46 (fresh this pass). |
