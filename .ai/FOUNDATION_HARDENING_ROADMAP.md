...
| G.2 | **No concurrency-control convention.** ~5 `version Int` fields across 645 tables — lost-update protection is effectively absent. | Platform optimistic-locking convention: `version` on every mutable business aggregate, enforced in the shared update helper (reject stale writes with 409 + standard error code); scaffolder emits it by default. |
...
| G.6 | **No boot-time env validation.** No schema-validated config; misconfiguration surfaces at first use, at runtime. | Zod-validated typed config loaded at bootstrap; process refuses to start on invalid/missing env; `.env.example` generated from the schema so it can never drift. |
...
| G.8 | **Money-type audit.** 744 `Decimal` vs 5 stray `Float` columns. | Audit the 5 `Float`s; migrate any monetary/quantity field to `Decimal`; add a schema lint that forbids `Float` for money/qty going forward. |
| G.9 | **Error envelope + pagination as executable contracts.** One `all-exceptions.filter.ts` exists; pagination is convention-only (rule 16b). | Freeze the error envelope (code, message, correlationId, details[]) and the list contract (`page/limit/sortBy/sortOrder` + `total`) as *typed, shared, tested* contracts in `packages/shared`, consumed by generated API types — not prose rules. |
...
| H.3 | **No backup/DR automation in the repo.** `scripts/` has no backup, restore-verify, or PITR drill; Phase 19.4 promised them. | Automated backup + **restore-verification** job (a backup that has never been restored is not a backup); documented RPO/RTO; quarterly PITR drill with runbook. Interlocks with Track A baselines. |
...
| Item | Verified gap | Required control |
|---|---|---|
| I.1 | **Production build is broken at HEAD** (`next build` fails on ui-* dist `.mjs` resolution; dev-only works). | A repo that cannot produce a production artifact has no deployable foundation. Fix root cause (package `exports`/build order), then make `build` a required CI gate so it can never silently break again. |
