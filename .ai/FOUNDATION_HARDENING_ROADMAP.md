...
|---|---|---|
| G.1 | ~~**API versioning is a prefix, not a policy.**~~ ✅ **CLOSED 2026-07-18 (cycle 10).** | `docs/API_VERSIONING_POLICY.md` (sealed contract: URI majors, side-by-side ≥1 release/6mo, mechanical deprecation clocks, ext `apiVersion` two-version window interplay, per-deprecation checklist) + live mechanism: `common/versioning/` registry (longest-prefix, empty today) + middleware emitting RFC 9745 `Deprecation` / RFC 8594 `Sunset` / successor `Link` headers, wired in `main.ts`; 5 unit tests. |
