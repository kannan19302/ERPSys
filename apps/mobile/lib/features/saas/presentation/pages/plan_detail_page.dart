  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          PermissionGate(
            permission: Permissions.adminSettingUpdate,
            child: IconButton(
              ...
              onPressed: () async {
                ...
                final r = await ref
                    .read(saasPlanListControllerProvider.notifier)
                    .delete(planId);
      ...
      body: async.when(
        ...
        error: (e, _) => FailureView(
            failure:
                e is Failure ? e : const ServerFailure('Could not load plan.'),
            onRetry: () => ref.invalidate(saasPlanDetailProvider(planId))),
      ...
      children: [
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: plan.isActive ? t.successLight : t.bgSunken,
                        borderRadius: Radii.pill),
                    child: Text(plan.isActive ? 'Active' : 'Inactive',
                        style: TextStyle(
                            color: plan.isActive ? t.success : t.textSecondary,
        ...
        _SectionCard(
          child: Column(
            ...
            children: [
              ...
              _FieldRow('Price', Formatters.currency(plan.price)),
              _FieldRow('Billing', plan.billingInterval),
            ],
          ),
        ),
        const SizedBox(height: Spacing.x4),
        _SectionCard(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const _SectionTitle(title: 'Limits'),
              _FieldRow('Max Users', plan.maxUsers?.toString() ?? '—'),
              _FieldRow('Max Storage',
                ...
                ...plan.features.map(
                  (f) => Padding(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.check,
                            color: t.success, size: TypeScale.base),
                        const SizedBox(width: Spacing.x2),
                        Expanded(
                            child: Text(f, style: TextStyle(color: t.text))),
                      ],
                    ),
                  ),
                ),
...
class _SaasPlanFormPageState extends ConsumerState<SaasPlanFormPage> {
  ...
  Future<void> _load() async {
    ...
    if (p != null) {
      ...
      _maxStorageCtrl.text = p.maxStorage?.toString() ?? '';
      _featuresCtrl.text = p.features.join(', ');
      _billingInterval = p.billingInterval;
  ...
  Future<void> _save() async {
    ...
    final payload = <String, dynamic>{
      ...
      'isActive': _isActive,
      'maxUsers': int.tryParse(_maxUsersCtrl.text),
      'maxStorage': int.tryParse(_maxStorageCtrl.text),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          AppBar(title: Text(_isEditing ? 'Edit Plan' : 'New Plan'), actions: [
        TextButton(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                    child: TextFormField(
                        controller: _maxStorageCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Max Storage (GB)')))
              ],
            ),
            const SizedBox(height: Spacing.x4),
            TextFormField(
...
class SaasSubscriptionDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
        ...
        body: ListView(
            padding: const EdgeInsets.all(Spacing.x4),
            children: [Text('Subscription: $subscriptionId')]));
  }
}
...
class _BillingFormPageState extends ConsumerState<BillingFormPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Billing Info'), actions: [
        TextButton(onPressed: _saving ? null : () {}, child: const Text('Save'))
      ]),
      body: Form(
        key: _formKey,
        child: ListView(
