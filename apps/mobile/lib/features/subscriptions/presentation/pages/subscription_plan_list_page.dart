...
class SubscriptionPlanListPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SubscriptionPlanListPage> createState() =>
      _SubscriptionPlanListPageState();
}

class _SubscriptionPlanListPageState
    extends ConsumerState<SubscriptionPlanListPage> {
  @override
  Widget build(BuildContext context) {
    final state = ref.watch(subscriptionPlanListControllerProvider);
    final controller =
        ref.read(subscriptionPlanListControllerProvider.notifier);
  ...
  Widget _body(SubscriptionPlanListState state,
      SubscriptionPlanListController controller, Palette t) {
                ...
                children: [
                  if (plan.trialDays > 0)
                    Text(
                      ...
                      style: TextStyle(
                          fontSize: TypeScale.xs, color: t.textTertiary),
                    ),
                  const Spacer(),
                  Text(
                    '#${plan.sortOrder}',
