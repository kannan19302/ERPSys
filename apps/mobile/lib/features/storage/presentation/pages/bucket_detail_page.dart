      ...
      body: async.when(
        ...
        error: (e, _) => FailureView(
            failure: e is Failure
                ? e
                : const ServerFailure('Could not load bucket.'),
            onRetry: () =>
                ref.invalidate(storageBucketDetailProvider(bucketId))),
...
class _BucketDetail extends StatelessWidget {
  const _BucketDetail({required this.bucket});
        ...
        _SectionCard(
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.cloud_outlined, color: t.primary, size: 40),
        ...
        _SectionCard(
          child: Column(
            ...
            children: [
              ...
              _FieldRow('Provider', bucket.provider),
              _FieldRow('Region', bucket.region),
              _FieldRow('Quota', '${bucket.maxQuotaGb} GB'),
              _FieldRow(
                  'Used', '${bucket.currentSizeGb.toStringAsFixed(2)} GB'),
              _FieldRow('Public', bucket.isPublic ? 'Yes' : 'No'),
              _FieldRow(
                  'Versioning', bucket.versioning ? 'Enabled' : 'Disabled'),
            ],
          ),
        ),
        const SizedBox(height: Spacing.x4),
          ...
          child: Column(
            ...
            children: [
              ...
              if (bucket.createdAt != null)
                _FieldRow('Created', Formatters.dateTime(bucket.createdAt!)),
              if (bucket.updatedAt != null)
                _FieldRow('Updated', Formatters.dateTime(bucket.updatedAt!)),
            ],
          ),
        ),
      ],
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        error: (e, _) => FailureView(
            failure:
                e is Failure ? e : const ServerFailure('Could not load file.'),
            onRetry: () => ref.invalidate(storageFileDetailProvider(fileId))),
            ...
            children: [
              ...
              _FieldRow(
                  ...
                  file.size > 0
                      ...
                      : '—'),
              _FieldRow('MIME Type', file.mimeType ?? '—'),
              _FieldRow('Bucket', file.bucket),
          ...
          child: Column(
            ...
            children: [
              ...
              if (file.updatedAt != null)
                _FieldRow('Updated', Formatters.dateTime(file.updatedAt!)),
            ],
          ),
        ),
      ],
