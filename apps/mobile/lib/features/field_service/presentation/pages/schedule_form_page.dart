...
class _ScheduleFormPageState extends ConsumerState<ScheduleFormPage> {
  ...
  Future<void> _loadExisting() async {
    final schedule =
        await ref.read(serviceScheduleDetailProvider(widget.id!).future);
    if (!mounted) return;
  ...
  Future<void> _submit() async {
    ...
    final result = await ref
        .read(serviceScheduleListControllerProvider.notifier)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          title: Text(widget.id != null ? 'Edit Schedule' : 'New Schedule')),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            UiTextField(
                label: 'Ticket Number', controller: _ticketNumberController),
            UiTextField(
              label: 'Technician ID',
            ...
            UiTextField(
                label: 'Customer Name', controller: _customerNameController),
            UiDropdownField(
              label: 'Status',
