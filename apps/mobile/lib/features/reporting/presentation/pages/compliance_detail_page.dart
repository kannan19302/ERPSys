      ...
      body: async.when(
        ...
        error: (e, _) => FailureView(
          failure: e is Failure
              ? e
              : const ServerFailure('Could not load compliance record.'),
          onRetry: () =>
              ref.invalidate(reportComplianceDetailProvider(complianceId)),
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              if (compliance.lastRunAt != null)
                _FieldRow(
                    'Last Run', Formatters.dateTime(compliance.lastRunAt!)),
              if (compliance.nextRunAt != null)
                _FieldRow(
                    'Next Run', Formatters.dateTime(compliance.nextRunAt!)),
              if (compliance.createdAt != null)
                _FieldRow(
                    'Created', Formatters.dateTime(compliance.createdAt!)),
            ],
          ),
        ),
              ...
              child: Row(
                children: [
                  Icon(Icons.warning_amber_rounded,
                      color: t.danger, size: Spacing.x5),
