...
class ListPushSubscriptionsUseCase
    extends UseCase<Cacheable<Paginated<PwaPushSubscription>>, ListQuery> {
  const ListPushSubscriptionsUseCase(this._repository);
  final PwaRepository _repository;
  @override
  Future<Result<Cacheable<Paginated<PwaPushSubscription>>>> call(
          ListQuery params) =>
...
class DeletePushSubscriptionUseCase extends UseCase<void, String> {
  ...
  Future<Result<void>> call(String id) =>
      _repository.deletePushSubscription(id);
}
...
class GetManifestConfigUseCase extends UseCase<PwaManifestConfig, NoParams> {
  ...
  final PwaRepository _repository;
  @override
  Future<Result<PwaManifestConfig>> call(NoParams params) =>
      _repository.getManifestConfig();
}

class UpdateManifestConfigUseCase
    extends UseCase<PwaManifestConfig, Map<String, dynamic>> {
...
class ListOfflineQueueUseCase
    extends UseCase<Cacheable<Paginated<PwaOfflineQueueItem>>, ListQuery> {
  const ListOfflineQueueUseCase(this._repository);
  final PwaRepository _repository;
  @override
  Future<Result<Cacheable<Paginated<PwaOfflineQueueItem>>>> call(
          ListQuery params) =>
      _repository.listOfflineQueue(params);
}

class RetryOfflineQueueItemUseCase
    extends UseCase<PwaOfflineQueueItem, String> {
