...
class AdminSettingsListPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminSettingsListPage> createState() =>
      _AdminSettingsListPageState();
}
...
class _AdminSettingsListPageState extends ConsumerState<AdminSettingsListPage> {
  ...
  Widget build(BuildContext context) {
    final AdminSettingListState state =
        ref.watch(adminSettingListControllerProvider);
    final AdminSettingListController controller =
        ref.read(adminSettingListControllerProvider.notifier);
              ...
              children: <Widget>[
                Expanded(
                  child: Text(
                    state.isLoading
                        ? 'Loading\u2026'
                        : '${state.meta.total} setting${state.meta.total == 1 ? '' : 's'}',
                    style: TextStyle(
  ...
  Widget _body(AdminSettingListState state,
      AdminSettingListController controller, Palette t) {
    if (state.isLoading && state.items.isEmpty) return const LoadingView();
    final Failure? failure = state.failure;
    if (failure != null && state.items.isEmpty)
      return FailureView(failure: failure, onRetry: controller.refresh);

    final Map<String, List<AdminSetting>> grouped =
        <String, List<AdminSetting>>{};
