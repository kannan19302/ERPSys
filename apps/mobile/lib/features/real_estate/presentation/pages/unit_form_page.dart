...
class _UnitFormPageState extends ConsumerState<UnitFormPage> {
  ...
  Future<void> _save() async {
    ...
    final result = await ref
        ...
        .saveUnit(payload, id: widget.unitId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          AppBar(title: Text(_isEditing ? 'Edit Unit' : 'New Unit'), actions: [
        TextButton(
          ...
          children: [
            TextFormField(
                ...
                decoration: const InputDecoration(
                    labelText: 'Unit Label *', hintText: 'e.g. 101'),
                validator: (v) =>
              ...
              children: [
                Expanded(
                    child: TextFormField(
                        ...
                        decoration:
                            const InputDecoration(labelText: 'Area (sqft)'))),
...
class _TenantFormPageState extends ConsumerState<TenantFormPage> {
  ...
  Future<void> _save() async {
    ...
    final result = await ref
        .read(propertyListControllerProvider.notifier)
        .saveTenant(payload, id: widget.tenantId);
    if (!context.mounted) return;
    setState(() => _saving = false);
    result.fold(
        (f) => ScaffoldMessenger.of(context)
