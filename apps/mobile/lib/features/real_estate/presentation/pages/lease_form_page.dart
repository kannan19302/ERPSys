...
class _LeaseFormPageState extends ConsumerState<LeaseFormPage> {
  ...
  Future<void> _save() async {
    ...
    final payload = <String, dynamic>{
      'leaseNumber': _leaseNumberCtrl.text.trim(),
      'propertyId': widget.propertyId,
      'monthlyRent': double.tryParse(_monthlyRentCtrl.text) ?? 0,
      'securityDeposit': double.tryParse(_depositCtrl.text) ?? 0,
      'currency': _currency,
      'paymentDay': int.tryParse(_paymentDayCtrl.text) ?? 1,
      'status': _status,
      'startDate': _startDate?.toIso8601String(),
      'endDate': _endDate?.toIso8601String(),
      'renewalTerms': _renewalTermsCtrl.text.trim().isEmpty
          ? null
          : _renewalTermsCtrl.text.trim(),
      'notes': _notesCtrl.text.trim().isEmpty ? null : _notesCtrl.text.trim(),
    };
    final result = await ref
        .read(propertyListControllerProvider.notifier)
        .saveLease(payload, id: widget.leaseId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: Text(_isEditing ? 'Edit Lease' : 'New Lease'),
          actions: [
            TextButton(
        ...
        child: ListView(
          ...
          children: [
            TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Lease Number *'),
                validator: (v) =>
              ...
              children: [
                Expanded(
                    child: TextFormField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'Payment Day'))),
            ...
            const SizedBox(height: Spacing.x4),
            TextFormField(
                controller: _renewalTermsCtrl,
                maxLines: 2,
                decoration: const InputDecoration(labelText: 'Renewal Terms')),
            const SizedBox(height: Spacing.x4),
            TextFormField(
