    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        error: (e, _) => FailureView(
            failure:
                e is Failure ? e : const ServerFailure('Could not load lease.'),
...
class _LeaseDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isExpired =
        lease.endDate != null && lease.endDate!.isBefore(DateTime.now());
    ...
    return ListView(
      ...
      children: [
                  ...
                  Column(
                    children: [
                      ...
                      if (isExpired)
                        Text('Overdue',
                            style: TextStyle(
                                color: t.danger, fontSize: TypeScale.xs)),
                    ],
                  ),
                ],
              ),
          ...
          child: Column(
            ...
            children: [
              ...
              if (lease.startDate != null && lease.endDate != null)
                _FieldRow('Duration',
                    '${lease.endDate!.difference(lease.startDate!).inDays ~/ 30} months'),
            ],
          ),
        ),
        const SizedBox(height: Spacing.x4),
        _SectionCard(
          child: Column(
            ...
            children: [
              ...
              _FieldRow(
                  'Monthly Rent',
                  Formatters.currency(lease.monthlyRent,
                      currencyCode: lease.currency)),
              _FieldRow(
                  'Security Deposit',
                  Formatters.currency(lease.securityDeposit,
                      currencyCode: lease.currency)),
              _FieldRow('Payment Day', '${lease.paymentDay}'),
              if (lease.renewalTerms != null)
                _FieldRow('Renewal Terms', lease.renewalTerms!),
            ],
          ),
        ),
        if (lease.createdAt != null) ...[
          ...
          _SectionCard(
              child:
                  _FieldRow('Created', Formatters.dateTime(lease.createdAt!))),
        ],
      ],
    );
  }
