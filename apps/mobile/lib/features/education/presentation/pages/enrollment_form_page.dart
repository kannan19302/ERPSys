...
class _EnrollmentFormPageState extends ConsumerState<EnrollmentFormPage> {
  ...
  void initState() {
    ...
    if (_isEditing) {
      final e =
          ref.read(enrollmentDetailProvider(widget.enrollmentId!)).valueOrNull;
      if (e != null) {
        _studentIdCtrl.text = e.studentId;
        _courseIdCtrl.text = e.courseId;
        _semesterCtrl.text = e.semester ?? '';
        _academicYearCtrl.text = e.academicYear ?? '';
        _status = e.status;
      }
    }
  }
  ...
  Future<void> _save() async {
    ...
    final result =
        await ref.read(enrollmentListControllerProvider.notifier).save(
      {
        ...
        'academicYear': _academicYearCtrl.text.trim().isEmpty
            ? null
