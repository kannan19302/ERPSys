...
class _StudentFormPageState extends ConsumerState<StudentFormPage> {
  ...
  Future<void> _loadStudent() async {
    final Student? s =
        ref.read(studentDetailProvider(widget.studentId!)).valueOrNull;
    if (s != null) {
      ...
      _enrollmentNumberCtrl.text = s.enrollmentNumber ?? '';
      _guardianNameCtrl.text = s.guardianName ?? '';
      _guardianPhoneCtrl.text = s.guardianPhone ?? '';
      _status = s.status;
      _gender = s.gender ?? 'NOT_SPECIFIED';
  ...
  Future<void> _save() async {
    ...
    final payload = <String, dynamic>{
      ...
      'enrollmentNumber': _enrollmentNumberCtrl.text.trim().isEmpty
          ? null
          : _enrollmentNumberCtrl.text.trim(),
      'guardianName': _guardianNameCtrl.text.trim().isEmpty
          ? null
          : _guardianNameCtrl.text.trim(),
      'guardianPhone': _guardianPhoneCtrl.text.trim().isEmpty
          ? null
    ...
    final result = await ref
        ...
        .save(payload, id: widget.studentId);
                  ...
                  child: TextFormField(
                    ...
                    controller: TextEditingController(
                      text: _dateOfBirth != null
                          ? '${_dateOfBirth!.toLocal()}'.substring(0, 10)
                          : '',
                    ),
                  ),
                ),
