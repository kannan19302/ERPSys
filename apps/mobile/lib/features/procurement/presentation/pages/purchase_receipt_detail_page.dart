      ...
      body: async.when(
        ...
        error: (error, _) => FailureView(
          failure: error is Failure
              ? error
              : const ServerFailure('Could not load receipt.'),
          onRetry: () =>
              ref.invalidate(purchaseReceiptDetailProvider(receiptId)),
        ),
            ...
            children: [
              Row(
                children: [
                  ...
                  UiStatusBadge(
                      label: receipt.status, tone: _statusTone(receipt.status)),
                ],
              ),
                ...
                (item) => Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Ordered: ${item.orderedQuantity.toStringAsFixed(0)}  |  Received: ${item.receivedQuantity.toStringAsFixed(0)}',
                        style: TextStyle(
                            fontSize: TypeScale.xs, color: t.textSecondary),
                      ),
                      if (item.acceptedQuantity > 0 ||
                          item.rejectedQuantity > 0)
                        Text(
                          'Accepted: ${item.acceptedQuantity.toStringAsFixed(0)}  |  Rejected: ${item.rejectedQuantity.toStringAsFixed(0)}',
                          style: TextStyle(
                              fontSize: TypeScale.xs, color: t.textSecondary),
                        ),
                    ],
          ...
          child: Column(
            ...
            children: [
              ...
              if (receipt.poNumber != null)
                _Row('Purchase Order', receipt.poNumber!),
              if (receipt.warehouseName != null)
                _Row('Warehouse', receipt.warehouseName!),
              if (receipt.receivedDate != null)
                _Row('Received Date', Formatters.date(receipt.receivedDate!)),
              if (receipt.notes != null && receipt.notes!.isNotEmpty)
                _Row('Notes', receipt.notes!),
              if (receipt.createdAt != null)
                _Row('Created', Formatters.dateTime(receipt.createdAt!)),
            ],
          ),
        ),
      ],
