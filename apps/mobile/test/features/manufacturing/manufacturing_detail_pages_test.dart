...
class FakeDetailRepository implements ManufacturingRepository {
  ...
  @override
  Future<Result<MrpRun>> getMrpRun(String id) async => const Result<MrpRun>.ok(
        MrpRun(
          id: 'mrp1',
          productId: 'p1',
          productName: 'P1',
      ...
      listQualityInspections(ListQuery query) async =>
          const Result<Cacheable<Paginated<QualityInspection>>>.ok(
            Cacheable<Paginated<QualityInspection>>(
              value: Paginated<QualityInspection>(
                data: [],
                meta: PaginationMeta(
                  page: 1,
                  limit: 25,
  ...
  Future<Result<QualityInspection>> getQualityInspection(String id) async =>
      const Result<QualityInspection>.ok(
        QualityInspection(
          id: 'qi1',
      ...
      listEngineeringChangeOrders(ListQuery query) async =>
          const Result<Cacheable<Paginated<EngineeringChangeOrder>>>.ok(
            Cacheable<Paginated<EngineeringChangeOrder>>(
              value: Paginated<EngineeringChangeOrder>(
                data: [],
                meta: PaginationMeta(
                  page: 1,
                  limit: 25,
  ...
  Future<Result<EngineeringChangeOrder>> getEngineeringChangeOrder(
    String id,
      ...
      const Result<EngineeringChangeOrder>.ok(
...
void main() {
  group('WorkOrderDetailPage', () {
    ...
    testWidgets('shows produced quantity when greater than zero',
      ...
      repo.getWorkOrderFn =
          (String id) async => const Result<WorkOrder>.ok(_workOrderInProgress);
    ...
    testWidgets('renders completed status for a finished work order',
      ...
      repo.getWorkOrderFn =
          (String id) async => const Result<WorkOrder>.ok(_workOrderCompleted);
    ...
    testWidgets('shows BOM, Workstation, and Routing labels when ids present',
      ...
      repo.getWorkOrderFn = (String id) async => const Result<WorkOrder>.ok(
            WorkOrder(
              id: 'wo4',
    ...
    testWidgets('shows loading state while fetching', (tester) async {
      ...
      repo.getWorkOrderFn =
          (String id) async => Result<WorkOrder>.ok(await completer.future);
  ...
  group('BomDetailPage', () {
    ...
    testWidgets('shows loading state while fetching BOM', (tester) async {
      ...
      repo.getBomFn =
          (String id) async => Result<Bom>.ok(await completer.future);
    ...
    testWidgets('renders draft status tone', (tester) async {
      ...
      repo.getBomFn = (String id) async => const Result<Bom>.ok(
            Bom(
              id: 'bom2',
              name: 'BOM Beta',
              productId: 'p2',
