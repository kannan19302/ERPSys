...
export default function TicketsPage() {
  ...
  const createTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMsg('Ticket registered successfully.');
        setShowForm(false);
  ...
  const resolveTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMsg('Ticket marked as resolved.');
        setSelectedTicketId('');
        setResolution('');
        fetchData();
      }
    } catch {
      setMsg('Error resolving ticket.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 0.4s ease-out' }}>
      <PageHeader
        title="HR Helpdesk"
        description="Verify and resolve corporate employee questions, payroll disputes, and workplace benefit queries."
        breadcrumbs={[{ label: 'Home', href: '/dashboard' }, { label: 'HR', href: '/hr' }, { label: 'Advanced', href: '/hr/advanced' }, { label: 'Helpdesk' }]}
        actions={
      ...
      {showForm && (
        <Card padding="md">
          <h4 style={{ margin: '0 0 var(--space-3)' }}>Record Employee Query Ticket</h4>
          <form onSubmit={createTicket} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-2)', justifyContent: 'flex-end' }}>
              ...
              <Button variant="primary" type="submit" disabled={submitting}>File Ticket</Button>
            </div>
          </form>
        </Card>
      )}
