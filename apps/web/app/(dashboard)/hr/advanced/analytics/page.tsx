...
export default function AnalyticsPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [headRes, compRes, costRes] = await Promise.all([
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 'var(--space-4)' }}>
            ...
            <Card>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-secondary)', fontWeight: 'var(--weight-medium)' }}>Average Monthly Salary</span>
              ...
              <h2 style={{ margin: 'var(--space-2) 0 0', fontSize: 'var(--text-2xl)' }}>
                ${Math.round(compensation?.average || 0).toLocaleString()}
              </h2>
            </Card>
