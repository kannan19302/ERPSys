...
import { CalendarDays, Play, RefreshCw, CheckCircle, Pause, PlayCircle } from 'lucide-react';
...
export default function ScheduledTasksPage() {
  ...
  const handleTriggerTask = async (id: string) => {
    ...
    setFeedback(null);
    try {
      const res = await fetch(`/api/v1/admin/operations/tasks/${id}/trigger`, {
        method: 'POST',
        headers: getHeaders(),
      });
      if (res.ok) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 'var(--weight-bold)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <CalendarDays style={{ color: 'var(--color-primary)' }} />
            Scheduled Tasks & Cron Manager
          </h1>
          <p style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--text-sm)' }}>
            Monitor and execute tenant-wide recurring cron schedules and daemon jobs.
          </p>
        </div>
              ...
              <tr key={t.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={{ padding: 'var(--space-4)', textAlign: 'center' }}>
                  <span style={{
                    ...
                    background: t.status === 'ACTIVE' ? 'rgba(var(--color-success-rgb), 0.1)' : 'rgba(var(--color-text-secondary-rgb), 0.1)',
