...
import { ToggleLeft, ToggleRight, RefreshCw, CheckCircle, Flame } from 'lucide-react';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: 'var(--text-2xl)', fontWeight: 'var(--weight-bold)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <Flame style={{ color: 'var(--color-primary)' }} />
            Beta & Feature Flags Settings
          </h1>
          <p style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--text-sm)' }}>
            Manage experimental system rollouts, tenant-level performance configurations, and features in staging/preview.
          </p>
        </div>
                ...
                <tr key={idx} style={{ borderBottom: '1px solid var(--color-border)' }}>
                  ...
                  <td style={{ padding: 'var(--space-4)', textAlign: 'center' }}>
                    <span style={{
                      ...
                      background: f.enabled ? 'rgba(var(--color-success-rgb), 0.1)' : 'rgba(var(--color-text-secondary-rgb), 0.1)',
                      color: f.enabled ? 'var(--color-success)' : 'var(--color-text-secondary)'
                    }}>{f.enabled ? 'Active' : 'Disabled'}</span>
                  </td>
