          ...
          {approvals.map((app) => (
            <div key={app.id} style={{ padding: 'var(--space-4)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--color-bg)', gap: 'var(--space-3)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                {app.status === 'PENDING' ? (
                  <>
                    <button onClick={() => handleApprove(app.id, 'APPROVED')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-success)', padding: 'var(--space-1)' }} title="Approve">
                      <CheckCircle size={22} />
                    </button>
                    <button onClick={() => handleApprove(app.id, 'REJECTED')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-error)', padding: 'var(--space-1)' }} title="Reject">
                      <XCircle size={22} />
                    </button>
                  </>
                ) : (
                  <span style={{
                    fontSize: '10px', fontWeight: 'bold', padding: '2px 8px', borderRadius: 'var(--radius-full)',
