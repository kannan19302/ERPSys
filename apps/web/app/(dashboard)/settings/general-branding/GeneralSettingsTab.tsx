...
export default function GeneralSettingsTab() {
  ...
  const fetchDemoStatus = useCallback(async () => {
    try {
      const res = await fetch('/api/v1/admin/demo/status', { headers: authHeaders() });
  ...
  const handleLoadDemo = async () => {
    setDemoLoading(true);
    try {
      const res = await fetch('/api/v1/admin/demo/load', { method: 'POST', headers: authHeaders() });
  ...
  const handleRemoveDemo = async () => {
    ...
    setDemoLoading(true);
    try {
      const res = await fetch('/api/v1/admin/demo/load', { method: 'DELETE', headers: authHeaders() });
