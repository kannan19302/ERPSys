  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/v1/admin/platform/maintenance', {
        method: 'POST', headers: getHeaders(), body: JSON.stringify(maintenance),
      });
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginTop: 'var(--space-2)' }}>
              <button type="submit" disabled={saving} style={{ background: 'var(--color-primary)', color: '#fff', border: 'none', padding: 'var(--space-2) var(--space-5)', borderRadius: 'var(--radius-md)', cursor: saving ? 'wait' : 'pointer', fontSize: 'var(--text-sm)', fontWeight: 'var(--weight-semibold)' }}>{saving ? 'Saving Status...' : 'Apply Status Settings'}</button>
