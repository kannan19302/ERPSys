...
import { Layers, RefreshCw, CheckCircle, AlertTriangle, ArrowRight } from 'lucide-react';
...
export default function BackgroundJobsPage() {
  ...
  const handleRetryFailed = async () => {
    ...
    setFeedback(null);
    try {
      const res = await fetch('/api/v1/admin/operations/jobs/retry', {
        method: 'POST',
        headers: getHeaders(),
      });
      if (res.ok) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--text-sm)' }}>
            Monitor active BullMQ queues, view active / waiting worker cycles, and retry failed operations.
          </p>
        </div>
            ...
            {loading ? (
              ...
              <tr key={idx} style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <td style={{ padding: 'var(--space-4)', textAlign: 'center' }}>
                  <span style={{
                    padding: '2px 8px', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-xs)', fontWeight: 'var(--weight-bold)',
                    background: q.active > 0 ? 'rgba(var(--color-primary-rgb), 0.1)' : 'var(--color-bg)',
                    color: q.active > 0 ? 'var(--color-primary)' : 'var(--color-text-secondary)'
                  }}>{q.active}</span>
                </td>
                <td style={{ padding: 'var(--space-4)', textAlign: 'center' }}>
                  <span style={{
                    padding: '2px 8px', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-xs)', fontWeight: 'var(--weight-semibold)',
                    background: q.waiting > 0 ? 'rgba(var(--color-warning-rgb), 0.1)' : 'var(--color-bg)',
                    color: q.waiting > 0 ? 'var(--color-warning)' : 'var(--color-text-secondary)'
                  }}>{q.waiting}</span>
                </td>
                <td style={{ padding: 'var(--space-4)', textAlign: 'center' }}>
                  <span style={{
                    padding: '2px 8px', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-xs)', fontWeight: 'var(--weight-semibold)',
                    background: 'rgba(var(--color-success-rgb), 0.1)', color: 'var(--color-success)'
                  }}>{q.completed}</span>
                </td>
                <td style={{ padding: 'var(--space-4)', textAlign: 'center' }}>
                  <span style={{
                    padding: '2px 8px', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-xs)', fontWeight: 'var(--weight-bold)',
                    background: q.failed > 0 ? 'rgba(var(--color-error-rgb), 0.1)' : 'var(--color-bg)',
                    color: q.failed > 0 ? 'var(--color-error)' : 'var(--color-text-secondary)'
                  }}>{q.failed}</span>
                </td>
              </tr>
            ))}
