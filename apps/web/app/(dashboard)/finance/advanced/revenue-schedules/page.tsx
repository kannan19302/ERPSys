  ...
  const columns: Column<RevenueSchedule>[] = [
    {
      ...
      render: (row) => (
        <div>
          <div style={{ fontWeight: 'var(--weight-semibold)', fontSize: 'var(--text-sm)' }}>{row.contractName}</div>
          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-tertiary)' }}>{row.startDate} → {row.endDate}</div>
        </div>
      ),
    },
    { key: 'method', header: 'Method', render: (row) => <Badge variant="info">{row.method.replace('_', ' ')}</Badge> },
    { key: 'totalAmount', header: 'Total', align: 'right' as const, render: (row) => <span style={{ fontWeight: 'var(--weight-semibold)' }}>{fmtCurrency(row.totalAmount)}</span> },
    {
      ...
      render: (row) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <div style={{ flex: 1, height: 6, borderRadius: 'var(--radius-full)', background: 'var(--color-bg-sunken)', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${pct}%`, background: 'var(--color-success)', borderRadius: 'var(--radius-full)' }} />
            </div>
            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-secondary)', minWidth: 32 }}>{pct}%</span>
          </div>
        );
      },
    },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      <PageHeader title="Revenue Recognition" description="Manage deferred revenue schedules and recognition rules"
        ...
        actions={<Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={14} style={{ marginRight: 6 }} /> New Schedule</Button>}
      />
