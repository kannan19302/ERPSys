...
export default function MaintenancePage() {
  ...
  useEffect(() => { (async () => { try { const res = await fetch('/api/v1/ext/real-estate/maintenances', { headers: { Authorization: `Bearer ${getToken() || ''}` } }); if (res.ok) { const d = await res.json(); setItems(Array.isArray(d) ? d : d?.data || []); } } catch {} finally { setLoading(false); } })(); }, []);
  const handleCreate = async () => { if (!form.propertyId) return; setCreating(true); try { await fetch('/api/v1/ext/real-estate/maintenances', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getToken() || ''}` }, body: JSON.stringify({...form, cost: Number(form.cost)}) }); setCreateOpen(false); window.location.reload(); } catch {} finally { setCreating(false); } };
