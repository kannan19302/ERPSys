...
export default function PropertyDetailPage() {
  ...
  useEffect(() => { if (!id) return; (async () => { try { const res = await fetch('/api/v1/real-estate/properties', { headers: { Authorization: `Bearer ${getToken() || ''}` } }); if (res.ok) { const d = await res.json(); const list = Array.isArray(d) ? d : d?.data || []; setProperty(list.find((p: Property) => p.id === id) || null); } } catch {} finally { setLoading(false); } })(); }, [id]);
