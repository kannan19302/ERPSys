...
export default function CommissionsPage() {
  ...
  useEffect(() => { (async () => { try { const res = await fetch('/api/v1/real-estate/commissions', { headers: { Authorization: `Bearer ${getToken() || ''}` } }); if (res.ok) { const d = await res.json(); setCommissions(Array.isArray(d) ? d : d?.data || []); } } catch {} finally { setLoading(false); } })(); }, []);
  const handleCreate = async () => { if (!form.agentId) return; setCreating(true); try { await fetch('/api/v1/real-estate/commissions', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getToken() || ''}` }, body: JSON.stringify({...form, amount: Number(form.amount), splitRatio: Number(form.splitRatio)}) }); setCreateOpen(false); window.location.reload(); } catch {} finally { setCreating(false); } };
