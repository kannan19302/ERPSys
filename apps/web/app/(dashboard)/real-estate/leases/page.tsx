...
export default function LeasesPage() {
  ...
  const columns: Column<Lease>[] = [
    { key: 'tenant', header: 'Tenant', render: (row) => (<div><Link href={`/real-estate/leases/${row.id}`} style={{ fontWeight: 'var(--weight-semibold)', fontSize: 'var(--text-sm)', color: 'var(--color-primary)', textDecoration: 'none' }}>{row.tenantName}</Link><div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-tertiary)' }}>{row.property?.name || row.propertyId.slice(0, 8)}</div></div>) },
    { key: 'dates', header: 'Lease Period', render: (row) => <span style={{ fontSize: 'var(--text-sm)' }}>{row.startDate ? new Date(row.startDate).toLocaleDateString() : '—'} → {row.endDate ? new Date(row.endDate).toLocaleDateString() : '—'}</span> },
    { key: 'rent', header: 'Rent', align: 'right' as const, render: (row) => <span style={{ fontWeight: 'var(--weight-semibold)' }}>{fmtCurrency(row.rentAmount)}</span> },
  ...
  return (<div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
    <PageHeader title="Leases" description="Active lease agreements and rent tracking" breadcrumbs={[{ label: 'Real Estate', href: '/real-estate' }, { label: 'Leases' }]}
      actions={<Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={14} style={{ marginRight: 6 }} /> New Lease</Button>} />
