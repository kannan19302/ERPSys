...
export default function EducationPage() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'students' | 'courses' | 'timetables'>('dashboard');
  ...
  // Compute charts
  const courseCreditsData = useMemo(() => {
  ...
  const timetableByWeekdayData = useMemo(() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 0.4s ease-out' }}>
      {/* Header */}
      <PageHeader
        title="Education Academy Hub"
        description="Manage student registrations, academic course curriculums, school timetables, and library holdings."
        breadcrumbs={[{ label: 'Home', href: '/dashboard' }, { label: 'Education' }]}
        actions={
          <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
            <ViewSwitcher activeView={activeView} onViewChange={setActiveView} availableViews={['list', 'chart']} />
            <Button onClick={handleCreateCourse} variant="outline" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
              <BookOpen size={16} /> Create Course
            </Button>
            <Button onClick={handleCreateStudent} variant="primary" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'var(--space-4)' }}>
        <DashboardKPICard
          ...
          drillDown={{
            modalTitle: 'Student Registry',
            columns: [
              { key: 'enrollmentNumber', label: 'Enrollment No.' },
              { key: 'firstName', label: 'First Name' },
              { key: 'lastName', label: 'Last Name' },
              { key: 'dateOfBirth', label: 'DOB', render: (v: any) => { if (!v) return '—'; const d = new Date(v); return isNaN(d.getTime()) ? '—' : d.toLocaleDateString(); } }
            ],
            rows: students.map(s => ({ ...s }))
          }}
        />
        <DashboardKPICard
          ...
          drillDown={{
            modalTitle: 'Offered Courses',
            columns: [
              { key: 'code', label: 'Code' },
              { key: 'name', label: 'Course Name' },
              { key: 'credits', label: 'Credits' }
            ],
            rows: courses.map(c => ({ ...c }))
          }}
        />
        <DashboardKPICard
          title="Scheduled Timetables"
          value={String(timetables.length)}
          icon={<Library size={18} />}
          color="var(--color-info-text)"
          drillDown={{
            modalTitle: 'Scheduled Timetables',
            columns: [
              { key: 'courseCode', label: 'Course' },
              { key: 'weekday', label: 'Day' },
              { key: 'startTime', label: 'Start Time' },
              { key: 'endTime', label: 'End Time' },
              { key: 'room', label: 'Classroom' }
            ],
            rows: timetables.map(t => ({
              ...t,
              courseCode: t.course ? `${t.course.name} (${t.course.code})` : '—'
            }))
          }}
        />
        <DashboardKPICard
          title="Total Credit Weight"
          value={`${totalCredits} Credits`}
          icon={<Award size={18} />}
          color="var(--color-warning-text)"
          drillDown={{
            modalTitle: 'Course Credit Valuation',
            columns: [
              { key: 'code', label: 'Code' },
              { key: 'name', label: 'Course Name' },
              { key: 'credits', label: 'Credits Weight' }
            ],
            rows: courses.map(c => ({ ...c }))
          }}
        />
      ...
      {activeView === 'chart' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: 'var(--space-4)' }}>
          <DashboardChart
            title="Course Credits Weight"
            subtitle="Credits allocation per course module"
            data={courseCreditsData}
            config={{ xAxisKey: 'name', series: [{ dataKey: 'value', name: 'Credits', color: 'var(--color-primary)' }] }}
          ...
          <DashboardChart
            ...
            subtitle="Number of scheduled classes by weekday"
            data={timetableByWeekdayData}
            config={{ xAxisKey: 'name', series: [{ dataKey: 'value', name: 'Classes', color: '#10b981' }] }}
            defaultChartType="bar"
            allowedChartTypes={['bar', 'donut', 'pie']}
            height={280}
          />
              ...
              {activeTab === 'dashboard' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                  <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)' }}>
                    Welcome to the Education Academy Hub. Select a tab above to manage students, courses, or timetable rosters.
                  </p>
                </div>
              )}
