...
export default function SalesReturnsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [returnsRes, ordersRes] = await Promise.all([
        fetch('/api/v1/sales/returns', { headers }),
  ...
  const handleOrderChange = async (orderId: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/sales/orders/${orderId}`, { headers });
      if (res.ok) {
        const data: SalesOrderDetail = await res.json();
        setLineItems(
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 0.4s ease-out' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-4)' }}>
        <Card style={{ padding: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          ...
          <div>
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-secondary)', fontWeight: 'var(--weight-medium)', textTransform: 'uppercase' }}>Total Customer Returns</div>
        ...
        <Card style={{ padding: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          ...
          <div>
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-secondary)', fontWeight: 'var(--weight-medium)', textTransform: 'uppercase' }}>Credit Notes Issued</div>
      ...
      <Card style={{ padding: 'var(--space-4)' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 'var(--space-12)', color: 'var(--color-text-secondary)' }}>
            ...
            <div style={{ fontSize: 'var(--text-sm)' }}>Log returns to update inventory stock and release Credit Notes.</div>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--text-sm)' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-sunken)' }}>
                  ...
                  <th style={{ textAlign: 'left', padding: 'var(--space-3) var(--space-4)', fontWeight: 'var(--weight-semibold)', color: 'var(--color-text-secondary)' }}>Client Name</th>
                  <th style={{ textAlign: 'left', padding: 'var(--space-3) var(--space-4)', fontWeight: 'var(--weight-semibold)', color: 'var(--color-text-secondary)' }}>Sales Order</th>
                  ...
                  <th style={{ textAlign: 'right', padding: 'var(--space-3) var(--space-4)', fontWeight: 'var(--weight-semibold)', color: 'var(--color-text-secondary)' }}>Total Credit</th>
              ...
              <tbody>
                {filteredReturns.map((r) => (
                  ...
                  >
                    ...
                    <td style={{ padding: 'var(--space-3.5) var(--space-4)' }}>{r.customerName}</td>
                    <td style={{ padding: 'var(--space-3.5) var(--space-4)' }}>
                      <Badge variant="default">{r.orderNumber}</Badge>
                    </td>
      ...
      {selectedReturn && (
        <div style={{ position: 'fixed', top: 0, right: 0, width: '460px', height: '100vh', background: 'var(--color-bg-elevated)', boxShadow: 'var(--shadow-xl)', borderLeft: '1px solid var(--color-border)', padding: 'var(--space-6)', zIndex: 100, display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', transition: 'transform 0.3s ease-out' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)', marginTop: 'var(--space-1)' }}>Customer: {selectedReturn.customerName}</div>
          </div>
          ...
          <div style={{ borderTop: '1px solid var(--color-border)', borderBottom: '1px solid var(--color-border)', padding: 'var(--space-4) 0', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontWeight: 'var(--weight-semibold)' }}>{selectedReturn.orderNumber}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--color-text-secondary)' }}>Credit Note Issued:</span>
              <span style={{ fontWeight: 'var(--weight-semibold)', color: 'var(--color-success)' }}>Active (CN-SR)</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ color: 'var(--color-success-text)', fontWeight: 'var(--weight-bold)' }}>Restocked</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--color-text-secondary)' }}>Total Credit Amount:</span>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'var(--color-bg-overlay)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: '600px', maxHeight: '90vh', background: 'var(--color-bg-elevated)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-xl)', overflow: 'hidden', display: 'flex', flexDirection: 'column', animation: 'scaleUp 0.3s ease-out' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-4) var(--space-6)', borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-sunken)' }}>
              <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--weight-semibold)' }}>Log Customer Return & Issue Credit Note</h3>
            ...
            {modalSuccess ? (
              <div style={{ padding: 'var(--space-8)', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-secondary)' }}>The inventory items have been restocked, and a Credit Note was generated.</div>
              </div>
            ) : (
              <form onSubmit={handleCreateReturn} style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', overflowY: 'auto' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                  ...
                  <div className="frappe-form-group">
                    <label style={{ display: 'block', fontSize: 'var(--text-xs)', fontWeight: 'var(--weight-semibold)', marginBottom: 'var(--space-1.5)' }}>Select Sales Order</label>
                    <select
                      required
                      value={salesOrderId}
