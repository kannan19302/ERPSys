            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border rounded-lg">
                <thead className="bg-gray-50">
                  <tr>{['Appt #', 'Type', 'Carrier', 'Trailer', 'Scheduled', 'Door', 'Status', 'Gate Pass', 'Actions'].map(h =>
                ...
                <tbody>
                  {appointments.map(a => (
                    <tr key={a.id} className="border-t hover:bg-gray-50">
                      <td className="px-3 py-2 font-mono">{a.appointmentNumber}</td>
                      <td className="px-3 py-2"><span className={`px-2 py-0.5 rounded text-xs ${a.appointmentType === 'INBOUND' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'}`}>{a.appointmentType}</span></td>
                      <td className="px-3 py-2">{a.carrierName ?? '-'}</td>
                      <td className="px-3 py-2 font-mono text-xs">{a.trailerNumber ?? '-'}</td>
                      <td className="px-3 py-2 text-xs text-gray-500">{new Date(a.scheduledAt).toLocaleString()}</td>
                      <td className="px-3 py-2 text-xs">{a.dockDoor?.doorNumber ?? '-'}</td>
                      <td className="px-3 py-2"><span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[a.status] ?? ''}`}>{a.status}</span></td>
                      <td className="px-3 py-2 text-xs">{a.gatePass ? `✓ ${a.gatePass.passNumber}` : '-'}</td>
                      <td className="px-3 py-2 flex gap-1 flex-wrap">
                        {a.status === 'SCHEDULED' && <>
                          <button onClick={() => apptAction(a.id, 'check-in', {})} className="text-purple-600 underline text-xs">Check In</button>
                          <button onClick={() => apptAction(a.id, 'no-show')} className="text-gray-500 underline text-xs">No Show</button>
                          <button onClick={() => apptAction(a.id, 'cancel')} className="text-red-600 underline text-xs">Cancel</button>
                        </>}
                        {a.status === 'CHECKED_IN' && <>
                          <button onClick={() => apptAction(a.id, 'start-loading')} className="text-orange-600 underline text-xs">Start Loading</button>
                          <button onClick={() => apptAction(a.id, 'complete', {})} className="text-green-600 underline text-xs">Complete</button>
                        </>}
                        {a.status === 'LOADING' && (
                          <button onClick={() => apptAction(a.id, 'complete', {})} className="text-green-600 underline text-xs">Complete</button>
                        )}
                  ...
                  {appointments.length === 0 && <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-400">No appointments</td></tr>}
                </tbody>
              </table>
            </div>
          )}
        ...
        <div>
          ...
          {loading ? <div className="text-gray-500">Loading...</div> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm border rounded-lg">
                <thead className="bg-gray-50">
                  <tr>{['Trailer #', 'Location', 'Description', 'Qty', 'Arrived', 'Actions'].map(h =>
                    <th key={h} className="px-3 py-2 text-left font-medium">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {yardInventory.map(y => (
                    <tr key={y.id} className="border-t hover:bg-gray-50">
                      <td className="px-3 py-2 font-mono">{y.trailerNumber}</td>
                      <td className="px-3 py-2">{y.location}</td>
                      <td className="px-3 py-2 text-gray-600">{y.description ?? '-'}</td>
                      <td className="px-3 py-2">{y.qty ? `${y.qty} ${y.uom ?? ''}` : '-'}</td>
                      <td className="px-3 py-2 text-xs text-gray-500">{new Date(y.arrivedAt).toLocaleDateString()}</td>
                      <td className="px-3 py-2">
                        <button onClick={async () => {
                          try {
                            await apiFetch(`/inventory/yard-management/yard-inventory/${y.id}/depart`, { method: 'PATCH' });
                            loadYardInventory();
                          } catch (e: unknown) { setError(String(e)); }
                  ...
                  {yardInventory.length === 0 && <tr><td colSpan={6} className="px-3 py-8 text-center text-gray-400">No trailers in yard</td></tr>}
                </tbody>
              </table>
            </div>
          )}
