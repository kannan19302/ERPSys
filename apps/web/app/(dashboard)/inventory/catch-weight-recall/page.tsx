...
const BASE = '/api/inventory/catch-weight-recall';
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (tab === 'dashboard') {
        ...
        setConfigs(await apiFetch(`${BASE}/configs`));
      } else if (tab === 'recalls') {
        setRecalls(await apiFetch(`${BASE}/recalls`));
      }
    } catch (e: unknown) {
        ...
        <div className="bg-white rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                {['Product ID', 'Nominal Weight', 'Tolerance (+/-)', 'Pricing Basis', 'Status'].map(h => (
            ...
            <tbody className="divide-y">
              {configs.map(c => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-xs">{c.productId}</td>
                  <td className="px-4 py-3">{Number(c.nominalWeightKg).toFixed(3)} kg</td>
                  <td className="px-4 py-3">+{Number(c.tolerancePctPlus).toFixed(1)}% / -{Number(c.tolerancePctMinus).toFixed(1)}%</td>
                  <td className="px-4 py-3 text-gray-500">{c.pricingBasis}</td>
              ...
              {configs.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No catch-weight configurations</td></tr>
              )}
            </tbody>
          </table>
        </div>
