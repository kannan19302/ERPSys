...
import { PieChart, TrendingUp, DollarSign, Activity, Target, BarChart3, Users } from 'lucide-react';
...
export default function CrmReportsPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetch('/api/v1/crm/analytics/pipeline-funnel', { headers: h }).then(r => r.ok ? r.json() : {}).catch(() => ({})),
            fetch('/api/v1/crm/analytics/win-rate', { headers: h }).then(r => r.ok ? r.json() : {}).catch(() => ({})),
            fetch('/api/v1/crm/analytics/lead-source-breakdown', { headers: h }).then(r => r.ok ? r.json() : {}).catch(() => ({})),
            fetch('/api/v1/crm/opportunities', { headers: h }).then(r => r.ok ? r.json() : []).catch(() => []),
            fetch('/api/v1/crm/leads', { headers: h }).then(r => r.ok ? r.json() : []).catch(() => []),
    ...
    const totalPipeline = (data.opportunities || []).filter((o: any) => o.stage !== 'CLOSED_LOST').reduce((s: number, o: any) => s + Number(o.amount || 0), 0);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 0.4s ease-out' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-4)' }}>
                ...
                <Card><div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-secondary)' }}>Win Rate</div><h3 style={{ margin: 'var(--space-1) 0', color: 'var(--color-success)' }}><TrendingUp size={16} style={{ display: 'inline' }} /> {(data.winRate as any)?.winRate || 0}%</h3></Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-6)' }}>
                ...
                <Card padding="md">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                        {Object.entries(data.leadSources || {}).map(([source, count]: any) => (
