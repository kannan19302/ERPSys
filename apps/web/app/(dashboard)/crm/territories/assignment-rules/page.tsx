...
export default function TerritoryAssignmentRulesPage() {
  ...
  const columns: Column<TerritoryRule>[] = [
    ...
    { key: 'name', header: 'Rule', render: (r) => <span style={{ fontWeight: 'var(--weight-semibold)' }}>{r.name}</span> },
    ...
    { key: 'isActive', header: 'Status', render: (r) => (
      <button onClick={() => handleToggleActive(r)} style={{ border: 'none', background: 'none', cursor: 'pointer' }}>
        <Badge variant={r.isActive ? 'success' : 'default'}>{r.isActive ? 'Active' : 'Disabled'}</Badge>
      </button>
    ) },
    { key: 'actions', header: '', align: 'right', render: (r) => (
      <button title="Delete" onClick={() => handleDelete(r)} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--color-danger, #dc2626)' }}><Trash2 size={14} /></button>
    ) },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 0.4s ease-out' }}>
        ...
        actions={
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            <ProtectedComponent permission="crm.lead.update">
              <Button variant="secondary" onClick={handleReassignAll} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
      ...
      <Card>
        <div style={{ padding: 'var(--space-4) var(--space-6)', borderBottom: '1px solid var(--color-border)' }}>
          <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--weight-semibold)' }}>Rules (highest priority evaluated first)</h3>
        </div>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 'var(--space-12)', color: 'var(--color-text-secondary)' }}>
            ...
            <div style={{ fontWeight: 'var(--weight-semibold)' }}>No Assignment Rules Yet</div>
            <div style={{ fontSize: 'var(--text-sm)' }}>Create a rule to start auto-routing leads to territories.</div>
          </div>
        ) : (
      ...
      <Card>
        <div style={{ padding: 'var(--space-4) var(--space-6)', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
          <ListChecks size={16} />
          <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--weight-semibold)' }}>Recent Assignment Log</h3>
        </div>
        <div style={{ padding: 'var(--space-4) var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          {log.length === 0 && <div style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--text-sm)' }}>No assignment decisions recorded yet.</div>}
          {log.slice(0, 20).map((entry) => (
            <div key={entry.id} style={{ fontSize: 'var(--text-sm)', display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: 'var(--space-2)' }}>
              <span>{entry.entityType} {entry.entityId} — {entry.reason}</span>
              <span style={{ color: 'var(--color-text-secondary)' }}>{new Date(entry.createdAt).toLocaleString()}</span>
            </div>
          ))}
        </div>
      </Card>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'var(--color-bg-overlay)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: '520px', background: 'var(--color-bg-elevated)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-xl)', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-4) var(--space-6)', borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-sunken)' }}>
              <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--weight-semibold)' }}>New Assignment Rule</h3>
            ...
            <form onSubmit={handleCreate} style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              <div>
                ...
                <input required value={name} onChange={(e) => setName(e.target.value)} className="frappe-input" style={inputStyle} placeholder="e.g. EU Enterprise Routing" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                <div>
                  <label style={labelStyle}>Rule Type</label>
                  <select value={ruleType} onChange={(e) => setRuleType(e.target.value as TerritoryRule['ruleType'])} className="frappe-input" style={inputStyle}>
                ...
                <div>
                  ...
                  <select required value={territoryId} onChange={(e) => setTerritoryId(e.target.value)} className="frappe-input" style={inputStyle}>
              ...
              <div>
                ...
                <input type="number" value={priority} onChange={(e) => setPriority(Number(e.target.value))} className="frappe-input" style={inputStyle} />
              </div>
              {ruleType === 'GEOGRAPHY' && (
                <div>
                  <label style={labelStyle}>Countries (comma-separated, e.g. US,CA)</label>
                  <input value={countries} onChange={(e) => setCountries(e.target.value)} className="frappe-input" style={inputStyle} />
                </div>
              )}
              {ruleType === 'INDUSTRY' && (
                <div>
                  <label style={labelStyle}>Industries (comma-separated)</label>
                  <input value={industries} onChange={(e) => setIndustries(e.target.value)} className="frappe-input" style={inputStyle} />
                </div>
              )}
              {ruleType === 'COMPANY_SIZE' && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                  <div>
                    ...
                    <input type="number" value={minEmployees} onChange={(e) => setMinEmployees(e.target.value)} className="frappe-input" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Max Employees</label>
                    <input type="number" value={maxEmployees} onChange={(e) => setMaxEmployees(e.target.value)} className="frappe-input" style={inputStyle} />
                  </div>
                </div>
              )}
