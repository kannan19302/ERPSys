...
export default function BattlecardsTab() {
  ...
  if (loading) {
    return <div style={{ display: 'flex', justifyContent: 'center', padding: 'var(--spacing-10)' }}><Spinner /></div>;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-5)' }}>
      ...
      <div style={{ maxWidth: 400, position: 'relative' }}>
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search competitors..." style={{ paddingLeft: 36 }} />
      </div>
            ...
            <div key={field} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-1)' }}>
              ...
              {form[field].map((val, i) => (
                <div key={i} style={{ display: 'flex', gap: 'var(--spacing-2)', alignItems: 'center' }}>
                  <Input value={val} onChange={(e) => updateRow(field, i, e.target.value)} placeholder={`Add ${field.slice(0, -1)}...`} style={{ flex: 1 }} />
