    ...
    return (
      <div style={{ gridColumn: `span ${field.columnSpan || 12}` }}>
        {isLayoutBreak ? (
          ...
          <div className="frappe-form-group" style={{ marginBottom: 0, height: '100%', display: 'flex', flexDirection: 'column' }}>
            {field.type !== 'Button' && field.type !== 'HTML' && (
              <label className="frappe-label" style={{ marginBottom: 'var(--space-1.5)' }}>
    ...
    >
      ...
      >
        <div style={{ position: 'absolute', right: '4px', top: '4px', display: 'flex', gap: '4px', zIndex: 5 }}>
           <button onClick={(e) => { e.stopPropagation(); removeField(field.id); }} className="frappe-btn frappe-btn-icon" style={{ padding: '4px', height: 'auto', background: 'var(--color-bg-sunken)', color: 'var(--color-text-tertiary)', border: 'none' }}><Trash2 size={12} /></button>
        ...
        {isLayoutBreak ? (
          ...
          <div style={{ opacity: 0.8, pointerEvents: 'none', flex: 1 }}>
            <label className="frappe-label" style={{ marginBottom: 'var(--space-1.5)', display: 'flex', alignItems: 'center', gap: 'var(--space-1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', paddingRight: '40px' }}>
              {field.label} 
