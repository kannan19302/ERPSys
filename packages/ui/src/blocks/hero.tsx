    ...
    <section className="hero-block" style={{ padding: '80px 20px', textAlign: alignment }}>
      <div className="container" style={{ maxWidth: 800, margin: alignment === 'center' ? '0 auto' : '0' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: alignment === 'center' ? 'center' : 'flex-start' }}>
          <a href={primaryUrl} className="frappe-btn frappe-btn-primary" style={{ padding: '12px 24px', fontSize: '1.1rem' }}>{primaryCta}</a>
          <a href={secondaryUrl} className="frappe-btn frappe-btn-default" style={{ padding: '12px 24px', fontSize: '1.1rem' }}>{secondaryCta}</a>
        </div>
      </div>
    </section>
  );
