...
export type ChartType = 'bar' | 'line' | 'area' | 'pie' | 'donut' | 'radar' | 'stacked-bar' | 'composed' | 'funnel';
          ...
          <div style={{
            ...
            {options.map((opt) => (
              <button
                key={opt.type}
                type="button"
                onClick={() => { onChange(opt.type); setIsOpen(false); }}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 'var(--space-2)',
                  ...
                  border: 'none',
                  borderRadius: 'var(--radius-md)',
                  background: opt.type === value ? 'var(--color-primary-light)' : 'transparent',
                  color: opt.type === value ? 'var(--color-primary)' : 'var(--color-text)',
                  fontSize: 'var(--text-sm)',
                  cursor: 'pointer',
                  textAlign: 'left',
                  fontWeight: opt.type === value ? 'var(--weight-semibold)' : 'normal',
                  transition: 'background var(--duration-fast)',
                }}
                onMouseEnter={(e) => {
                  if (opt.type !== value) e.currentTarget.style.background = 'var(--color-bg-sunken)';
                }}
                onMouseLeave={(e) => {
                  if (opt.type !== value) e.currentTarget.style.background = 'transparent';
                }}
              >
                <span>{opt.icon}</span>
                <span>{opt.label}</span>
